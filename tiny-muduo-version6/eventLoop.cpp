...
std::mutex EventLoop::_mt_conns;
std::list<TcpConn*> EventLoop::_conns;
std::unordered_map<int64_t, TcpConn*> EventLoop::_fd2conn;

EventLoop::EventLoop(): _tid(std::this_thread::get_id()){  // save the thread_id creating this eventlopp

    _callingPending = false;
...
void EventLoop::loop(){  //one loop per thread
    ...
    while(true){
        ...
        std::vector<CALLBACK> temp;
        {
            std::lock_guard<std::mutex> g_lock(_mt_pendingTask);
            
            _pendingTasks.swap(temp);

            _callingPending = true;            
        }
        for(auto& func : temp) func(); //do pending event
        _callingPending = false;
...
void EventLoop::enPendingQueue(CALLBACK&& cb){  //pendingevent wakeup eventfd, because no channel io will lead loop break but there are pendingevents want to run
    ...
    {
        std::lock_guard<std::mutex> g_lock(this->_mt_pendingTask);

        _pendingTasks.push_back(std::move(cb));
    }

    if(std::this_thread::get_id() != _tid || _callingPending){  //if thread is not cur eventloop creator, or there are other threads blocked, then wake up others
...
void EventLoop::runInLoop(CALLBACK&& cb){
    if(std::this_thread::get_id() == _tid) cb();
    else enPendingQueue(std::move(cb));
}


void EventLoop::addConn(int64_t fd, TcpConn* conn){
    std::lock_guard<std::mutex> g_lock(EventLoop::_mt_conns);
    EventLoop::_conns.push_back(conn);
    EventLoop::_fd2conn[fd] = conn;
}
...
void EventLoop::erase(int64_t fd){
    {
        ...
        EventLoop::_conns.remove(EventLoop::_fd2conn[fd]);
    }
