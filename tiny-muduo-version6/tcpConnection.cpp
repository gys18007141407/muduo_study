...
uint64_t TcpConn::send(std::string msg){  // split into two situation
    ...
    return msg.size(); //return value does not need?

}

uint64_t TcpConn::send2all(std::string msg){  

    std::lock_guard<std::mutex> g_lock(EventLoop::_mt_conns);

    for(auto& conn : EventLoop::_conns){
        if(conn != this){
            conn->send(msg);
        }
    }
...
uint64_t TcpConn::sendInLoop(std::string msg){
    ...
    std::cout << std::this_thread::get_id() << " ready to send message " << std::endl;
